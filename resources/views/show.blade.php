...
@section('body')
								...
								<th> </th>
									...
									@foreach($company->employees as $employ)
										<tr>
											...
											<td>{{$employ->phone}}</td>
											<td>
												<form action="/employees/{{$employ->id}}/edit"><button type="submit" class="btn btn-outline-dark">Edit</button></form>
	...
	<form action="/employees/{{ $company->id }}/create"><button type="submit" class="btn btn-success">Add new Employee</button></form>
